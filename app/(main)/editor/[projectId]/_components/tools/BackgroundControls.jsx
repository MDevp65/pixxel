...
export function BackgroundControls({ project }) {
  ...
  // set ai backtround
  const setAIBackground = async () => {
    if (!canvasEditor) return

    const mainImage = getMainImage();
    if (!mainImage || !project) return;

    setProcessingMessage("Applying background with AI...");
    ...
    try {
      ...
      // Create ImageKit transformation URL for background removal
      const aiBgAppliedUrl = currentImageUrl.includes("ik.imagekit.io")
        ? `${currentImageUrl.split("?")[0]}?tr=e-changebg-prompt-${aiPrompt.trim().toLocaleLowerCase()}`
        : currentImageUrl;

      // Create new image with background removed
      const processedImage = await FabricImage.fromURL(aiBgAppliedUrl, {
        crossOrigin: "anonymous",
      });
  ...
  // Handle search on Enter key for unsplash images
  ...
  // Handle search on Enter key for ai background
  const handleAiBgSearchKeyPress = (e) => {
    if (e.key === "Enter") {
      setAIBackground();
    }
  };
      ...
      <Tabs defaultValue="color" className="w-full">
        ...
        <TabsContent value="image" className="space-y-4 mt-6">
          ...
          <div>
            ...
            <p className="text-xs text-white/70 mb-4">
              Enter Prompt to set high quality images in background
            </p>
          </div>
          {/* Search Bar */}
          <div className="flex gap-2">
            <Input
              ...
              onKeyPress={handleAiBgSearchKeyPress}
              placeholder="Search for AI backgrounds..."
              className="flex-1 bg-slate-700 border-white/20 text-white"
            />
            <Button
              onClick={setAIBackground}
              disabled={processingMessage || !aiPrompt.trim()}
              variant="primary"
            >
